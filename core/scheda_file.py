...
def cartella_lavoro_per_bundle(
    percorso_bundle: str, *, base_dir: str | os.PathLike | None = None
) -> str:
    """
    ...
    return f"{_risolvi_percorso_bundle(percorso_bundle, base_dir)}.work"


def _risolvi_percorso_bundle(
    percorso_bundle: str | os.PathLike, base_dir: str | os.PathLike | None
) -> str:
    """Risolve i nomi bundle relativi rispetto alla directory esplicita."""
    percorso = os.fspath(percorso_bundle)
    if base_dir is not None and not os.path.isabs(percorso):
        return os.path.join(os.fspath(base_dir), percorso)
    return percorso
...
def carica_scheda(
    percorso_bundle: str, cartella_lavoro: str | None = None, *, base_dir: str | os.PathLike | None = None
) -> tuple[list[dict], str]:
    """
    ...
    if cartella_lavoro is None:
        cartella_lavoro = cartella_lavoro_per_bundle(percorso_bundle, base_dir=base_dir)
    percorso_bundle = _risolvi_percorso_bundle(percorso_bundle, base_dir)
